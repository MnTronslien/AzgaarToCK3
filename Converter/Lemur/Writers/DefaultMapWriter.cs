...
";
        ...
        Logger.Info($"Wrote default.map ({seaZoneList.Split(' ').Length} sea zones, {wastelandList.Split(' ').Length} impassable_mountains, {farSeaList.Split(' ').Length} impassable_seas, {(string.IsNullOrWhiteSpace(riverList) ? 0 : riverList.Split(' ').Length)} river_provinces)");
    }
}
