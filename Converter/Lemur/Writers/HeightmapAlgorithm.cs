...
static class HeightmapAlgorithm
{
    ...
    public static GenerateResult Generate(IReadOnlyDictionary<int, Cell> cells, Params p)
    {
        ...
        // ── 4. Relaxation ─────────────────────────────────────────────────────
        float avgPixelArea  = terrainNodes.Where(t => t.IsLand).Average(t => t.Area * scaleX * scaleY);
    ...
    static List<SpawnedNode> Relax(
        List<SpawnedNode> nodes, List<(float px, float py)> fixedRepulsors,
    ...
    static float[] Rasterize(Dictionary<(int, int), float> coordIndex, IEnumerable<Coordinate> points, Params p)
    {
