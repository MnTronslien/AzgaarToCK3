...
- **Province terrain (gameplay).** Every barony in the generated mod is assigned a CK3 gameplay terrain (`plains`, `hills`, `mountains`, `forest`, `desert`, …) that drives combat, movement, and supply modifiers. See [Province terrain in CONVERSION_RULES.md](docs/CONVERSION_RULES.md#province-terrain-gameplay) for what each rule does. All rules live in one file: `Converter/Lemur/Provinces/TerrainRegistry.cs` — each entry is a `(Ck3Terrain, score lambda)` pair and a change is usually a few lines (tweak a threshold, swap a biome → terrain mapping, raise a max to make a rule fire more aggressively). Tuning is encouraged; the current values were chosen on a single test map.
- **Terrain debug image.** Enable `GenerateDebugImages` in `settings.json` and re-run the converter. A `9_terrain_overview.png` lands under `%LOCALAPPDATA%\AzgaarToCK3\debug\<run>\` — each barony coloured by its assigned terrain, river network overlaid, province outlines drawn. Eyeball this before and after tweaking `TerrainRegistry.cs` to see what your change does without launching CK3.
