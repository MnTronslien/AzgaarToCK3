...
Two of the biggest gaps between "the map looks right" and "the map plays right" close here. Until now every generated realm was feudal and every county started at development 0 — the political map was uniform. This release derives each realm's **government** from its Azgaar state's political form (republics, theocracies, clans, tribes, and the DLC-gated nomad / administrative / Sōryō types) and seeds each county's **starting development** from its capital settlement's population, so the world begins with real political and economic texture instead of a flat feudal sheet.

### Added
- **Government types from Azgaar state form.** Every kingdom and duchy is now stamped with a CK3 government — `feudal`, `clan`, `tribal`, `republic`, `theocracy`, plus DLC-gated `nomad` (Khans of the Steppe), `administrative` (Roads to Power), and `japan_feudal`/Sōryō (All Under Heaven) — instead of every realm defaulting to feudal. Resolution reads Azgaar's `formName` first (granular: Emirate → clan, Most Serene Republic → republic, Shogunate → Sōryō, …), falls back to the broad `form` (Monarchy → feudal, Theocracy → theocracy, …), and finally to feudal. Kingdoms and duchies resolve independently from the state their cells belong to, so an absorbed Republic duchy under a Feudal king keeps its republic government — exactly how Venice and Genoa sit under feudal lieges in vanilla 1066. Emitted as a plain `government = X` on the title's 1066.1.1 history block; the engine handles DLC-missing fallback at game-start (nomad → tribal, administrative/Sōryō → feudal), so no conditional script is needed. Mapping table lives in `Converter/Lemur/Governments/GovernmentMap.cs` and is documented in `docs/CONVERSION_RULES.md`. Resolves issue #9 (MVP — duchy/kingdom tier; vassal-count government is a follow-up).
